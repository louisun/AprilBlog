...
class BlogPostAdminForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'snippet': Textarea(attrs={'rows': 10, 'cols': 100}),
            'content': Textarea(attrs={'rows': 10, 'cols': 100}),
...
class BlogPostAdmin(admin.ModelAdmin):
    fieldsets = [
        ('Info', {'fields': ['md_file', 'title', 'author', 'link', 'tags']}),
        ('Content', {'fields': ['snippet', 'content'], 'classes': ['collapse']}),
        ('Date information', {'fields': ['publish_time', 'update_time']}),
        ('Status', {'fields': ['status', 'is_public', 'is_top']})
    ...
    def delete_old_md_file():
        ...
        md_file_list = []
        for blogpost in Blog.objects.all():
            if blogpost.md_file:
    ...
    def save_model(self, request, obj, form, change):
        if obj:
            if obj.content:  # content有内容的时候才会更新md_file
                ...
                obj.md_file.save(filename + '.md', ContentFile(obj.content), save=False)
                obj.md_file.close()
        obj.save()
